...
function buildFormState(
  settings: SlackIntegrationSettings | undefined,
): SlackIntegrationSettings {
  return {
    enabled: settings?.enabled ?? false,
    delivery_method: settings?.delivery_method ?? "webhook",
...
export function SlackIntegrationPage() {
  ...
  const [form, setForm] = useState<SlackIntegrationSettings>(buildFormState(undefined));
  ...
  const updateField = <K extends keyof SlackIntegrationSettings>(
    key: K,
    value: SlackIntegrationSettings[K],
  ) => {
  ...
  const handleSave = async () => {
    setStatusMessage(null);
    const payload: SlackIntegrationSettings = {
      ...form,
    ...
    <div className="space-y-6">
      <section className="panel rounded-2xl p-6 sm:p-8">
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-white">Slack notifications</h2>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-400">
              Send AI recommendations from investigations and PR reviews to your preferred
              Slack channel. Configure a personal webhook or channel; instance-level defaults
              apply for GitHub webhook reviews when set by your admin.
            </p>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-5">
            <div>
              ...
              <select
                ...
                onChange={(event) =>
                  updateField("delivery_method", event.target.value as SlackDeliveryMethod)
                }
              ...
              >
                ...
                <option value="channel">Bot channel (requires server token)</option>
              </select>
            </div>
            ...
            {form.delivery_method === "webhook" ? (
              <div>
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Create an incoming webhook in Slack and paste the URL here. Leave blank to keep
              ...
              <div>
                ...
                />
                <p className="mt-2 text-xs text-slate-500">
                  {settings?.instance_bot_configured
                    ? "Uses the server SLACK_BOT_TOKEN to post to this channel."
                    : "Ask your admin to set SLACK_BOT_TOKEN in the backend environment."}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Choose which AI recommendations should be posted to Slack.
            </p>
      ...
      <section className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6">
        ...
        <ul className="mt-4 space-y-3 text-sm text-slate-400">
          <li>
            ...
            and validation steps are posted to your channel.
          </li>
          <li>
