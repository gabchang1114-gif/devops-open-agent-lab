...
from app.modules.aws.ai.confidence_engine import AwsConfidenceEngine
from app.modules.aws.ai.context_builder import AwsContextBuilder
from app.modules.aws.ai.fix_recommendation import AwsFixRecommendationEngine
from app.modules.aws.ai.prompt_builder import AwsPromptBuilder
from app.modules.aws.models import AwsInvestigationResponse


class AwsRootCauseAnalyzer:
    """Analyze AWS investigation evidence and produce structured diagnosis."""

    def __init__(self, settings: Settings | None = None) -> None:
        self.settings = settings or get_settings()
        self.context_builder = AwsContextBuilder()
        self.prompt_builder = AwsPromptBuilder()
        self.confidence_engine = AwsConfidenceEngine()
        self.fix_recommendation_engine = AwsFixRecommendationEngine()

    async def analyze(
        self,
        investigation: AwsInvestigationResponse | dict[str, Any],
    ) -> DiagnosisResult:
        ...
        try:
            ...
            diagnosis = self.confidence_engine.apply(diagnosis, context)
            diagnosis = self.fix_recommendation_engine.apply(diagnosis, context)
            logger.info(
                "AWS AI diagnosis generated | provider={} confidence={}",
    ...
    def _parse_response(self, raw_response: str) -> DiagnosisResult:
        ...
        try:
            ...
            logger.warning("AWS LLM JSON did not match schema, attempting recovery | error={}", exc)
    ...
    def _recover_partial_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for item in evidence if isinstance(evidence, list) else []:
            if isinstance(item, dict):
                normalized_evidence.append(
                    DiagnosisEvidence(
                        source=str(item.get("source", "ec2")),
        ...
        return {
            ...
            "suggested_fix": str(
                payload.get("suggested_fix", "Review investigation evidence manually.")
            ),
            ...
            "confidence_reason": str(
                payload.get("confidence_reason", "Recovered from partial LLM output.")
            ),
